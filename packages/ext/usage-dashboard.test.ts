...
describe("usage dashboard auxiliary records", () => {
  it.each([false, true])(
    "groups Auto Mode by reviewer and provider/model (Guardian: %s)",
    async (guardian) => {
      ...
      const autoMode = today.providers.get(guardian ? "Auto Mode (Guardian)" : "Auto Mode");

      expect(today.providers.has(guardian ? "Auto Mode" : "Auto Mode (Guardian)")).toBe(false);
      expect(data!.allTime.providers.size).toBe(guardian ? 3 : 2);
      ...
      if (guardian) component.handleInput("\x1b[B");
      component.handleInput("\r");
      const rendered = component.render(120).join("\n");
      expect(rendered).toContain(guardian ? "Auto Mode (Guardian)" : "Auto Mode");
