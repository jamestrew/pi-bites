...
describe("resolveModel", () => {
  ...
  describe("fuzzy match — trailing date-stamp is optional", () => {
    ...
    it("matches a dated provider/id config to an undated registry id", () => {
      expect(
        resolveModel("anthropic/claude-haiku-4-5-20251001", makeRegistry([HAIKU_DASH])),
      ).toEqual(HAIKU_DASH);
    });

    it("matches a dated config to an undated *dotted* registry id (date + separator)", () => {
      expect(
        resolveModel("anthropic/claude-haiku-4-5-20251001", makeRegistry([HAIKU_DOT])),
      ).toEqual(HAIKU_DOT);
    });
    ...
    it("still prefers an exact dated id when the registry has it", () => {
      ...
      expect(resolveModel("anthropic/claude-haiku-4-5-20251001", makeRegistry([dated]))).toEqual(
        dated,
      );
    });
  });
  ...
  describe("provider fallback (prefer named provider, else any)", () => {
    ...
    it("falls back to another provider when the named one lacks the model", () => {
      expect(resolveModel("anthropic/claude-haiku-4-5", makeRegistry([gatewayHaiku]))).toEqual(
        gatewayHaiku,
      );
    });
    ...
    it("prefers the named provider when it has the model", () => {
      expect(
        resolveModel("anthropic/claude-haiku-4-5", makeRegistry([gatewayHaiku, anthropicHaiku])),
      ).toEqual(anthropicHaiku);
    });

    it("still errors when no provider has the model", () => {
      expect(typeof resolveModel("anthropic/nonexistent-xyz", makeRegistry([gatewayHaiku]))).toBe(
        "string",
      );
    });
  });
