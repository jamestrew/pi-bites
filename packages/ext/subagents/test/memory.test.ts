...
describe("memory", () => {
  ...
  describe("resolveMemoryDir", () => {
    ...
    it("throws on names with path traversal (..)", () => {
      expect(() => resolveMemoryDir("../../etc/evil", "project", "/workspace")).toThrow(
        "Unsafe agent name",
      );
    });

    it("throws on names with forward slash", () => {
      expect(() => resolveMemoryDir("foo/bar", "project", "/workspace")).toThrow(
        "Unsafe agent name",
      );
    });
    ...
    it("throws on names with null byte", () => {
      expect(() => resolveMemoryDir("foo\0bar", "project", "/workspace")).toThrow(
        "Unsafe agent name",
      );
    });
    ...
    it("throws on names starting with dot", () => {
      expect(() => resolveMemoryDir(".hidden", "project", "/workspace")).toThrow(
        "Unsafe agent name",
      );
    });
