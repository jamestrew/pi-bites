...
describe("wait_agent", () => {
  ...
  it("retains unknown prototype-property target names in serialized status", async () => {
    const { completion } = harness([]);
    const result = await completion.waitFor(["__proto__", "constructor", "toString"], 30_000);
    expect(JSON.parse(JSON.stringify(result.status))).toEqual(
      Object.fromEntries(["__proto__", "constructor", "toString"].map((id) => [id, "not_found"])),
    );
    completion.dispose();
  });
