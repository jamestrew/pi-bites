...
export function sanitizeText(text: string): string {
  return (
    stripVTControlCharacters(
      // Node's ANSI stripper does not consistently remove OSC sequences across runtimes.
      // eslint-disable-next-line no-control-regex -- OSC sequences are terminal controls.
      text.replace(/(?:\u001b\]|\u009d)[\s\S]*?(?:\u0007|\u001b\\|\u009c|$)/g, ""),
    )
      .replaceAll("\t", "  ")
