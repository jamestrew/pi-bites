...
export function createWaitAgent(deps: WaitAgentDeps) {
  ...
  return defineSubagentTool({
    ...
    async execute(_toolCallId, params, signal, onUpdate) {
      ...
      try {
        ...
        if (outcome.outcome === "error")
          throw new SubagentOperationError(outcome.message, finalDetails);
        if (outcome.outcome === "cancelled")
          throw new SubagentOperationError("Agent wait cancelled", finalDetails);
        return v1Result({ status: outcome.status, timed_out: outcome.timed_out }, finalDetails);
      } finally {
    ...
    renderResult(result, { expanded }, theme, context) {
      let details = result.details as WaitAgentDetails | undefined;
      ...
      if (!details || !Array.isArray(details.agents)) return { render: () => [], invalidate() {} };
