...
export class SubagentScheduler {
  private jobs = new Map<string, { stop(): void; nextRun?(): Date | undefined }>();
  ...
  private scheduleJob(job: ScheduledSubagent): void {
    ...
    try {
      if (job.scheduleType === "interval" && job.intervalMs) {
        ...
        throw new Error(
          "Cron schedules are disabled in pi-bites for now; use interval or one-shot schedules.",
        );
      }
    } catch (err) {
      this.emit({
  ...
  static validateCronExpression(expr: string): { valid: boolean; error?: string } {
    ...
    try {
      // ponytail: cron execution is intentionally not vendored for #100; keep
      // parser shape so the feature can come back by restoring a cron engine.
      return { valid: /^[\d*/,-]+ [\d*/,-]+ [\d*/,-]+ [\d*/,-]+ [\d*/,-]+ [\d*/,-]+$/.test(expr) };
    } catch (e) {
