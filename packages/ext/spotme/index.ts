...
const SPOTME_TOOL_NAMES = ["spotme_status", "spotme_exercise", "spotme_end"] as const;
...
export default function (pi: ExtensionAPI) {
  ...
  function setSpotmeToolsActive(active: boolean): void {
  ...
  function requireSpotmeOn(ctx: {
    ui: { notify: (msg: string, type?: "error" | "info" | "warning") => void };
  }): boolean {
    if (engine.state.enabled) return true;
    ctx.ui.notify("SpotMe is off. Run `/spotme:on` first.", "warning");
    return false;
  }
  ...
  pi.on("session_start", async () => {
    if (!engine.state.enabled) setSpotmeToolsActive(false);
  });
  ...
  pi.registerCommand("spotme:hint", {
    ...
    handler: async (_args, ctx) => {
      ...
      const ex = engine.state.exercise;
      if (!ex?.active) {
        ctx.ui.notify("No active SpotMe exercise to hint.", "warning");
        return;
      }
