...
test.skipIf(!host)(
  ...
  async () => {
    ...
    const { join } = await import("node:path");
    const { tmpdir } = await import("node:os");
    const directory = mkdtempSync(join(tmpdir(), "code-mode-queued-approval-"));
    symlinkSync(host!, join(directory, "codex-code-mode-host"));
    vi.stubEnv("PATH", `${directory}:${process.env.PATH ?? ""}`);
    const firstChoice = Promise.withResolvers<string>();
    const secondChoice = Promise.withResolvers<string>();
    const gate = createBashGateHarness([], false, undefined, true, {
      bashGate: { rules: [{ cmd: "sleep" }] },
    });
    gate.ui.select
    ...
    try {
      await h.emit("session_start", {}, { ...context(), ui: gate.ui });
        ...
        calls.push(
          h.tools
            ...
            .execute(
              id,
              {
                code: '// @exec: {"yield_time_ms":25}\nawait tools.exec_command({cmd:"sleep 60",login:false,yield_time_ms:30000});',
              },
              undefined,
              (update: any) => {
                if (update.details.traces.some((trace: any) => trace.state === "approval"))
                  approving.add(id);
              },
            )
            .then((result: any) => {
      ...
      await expect.poll(() => approving.size).toBe(2);
      await expect.poll(() => gate.ui.select.mock.calls.length).toBe(1);
      await new Promise((resolve) => setTimeout(resolve, 100));
      expect(settled).toEqual([]);
      firstChoice.resolve("Allow");
      await expect.poll(() => gate.ui.select.mock.calls.length).toBe(2);
      await new Promise((resolve) => setTimeout(resolve, 100));
      expect(settled).toEqual([]);
      secondChoice.resolve("Allow");
      // Neither 60-second shell (nor its 30-second initial poll) should hold these responses.
      const results = await Promise.all(calls);
      expect(results.map((result) => result.details.state)).toEqual(["yielded", "yielded"]);
    } finally {
      firstChoice.resolve("Deny");
      secondChoice.resolve("Deny");
      await h.emit("session_shutdown", { reason: "quit" }, {});
