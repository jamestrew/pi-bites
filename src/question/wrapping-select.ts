...
export interface WrappingSelectItem {
  label: string;
  description?: string;
  /** Render a styled "★ Recommended" badge below the label. */
...
export class WrappingSelect implements Component {
  ...
  private renderDescriptionBlock(
    ...
    const lines: string[] = [];
    if (isRecommended) {
      lines.push(`${continuationPrefix}${this.theme.description("★ Recommended")}`);
    }
    if (description) {
      const wrapped = wrapTextWithAnsi(description, contentWidth);
      lines.push(
        ...wrapped.map((segment) => `${continuationPrefix}${this.theme.description(segment)}`),
      );
    }
    return lines;
  }
