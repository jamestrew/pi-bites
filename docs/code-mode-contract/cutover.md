...
Stock provider serialization checks compare initial/subsequent definitions and system instructions for grammar and structured fallback on Responses, Codex Responses, and Completions. Live payloads on both configured routes used grammar; every request within each scenario retained identical tool and system-instruction hashes. [Recorded evidence](smoke/discovery-results.json) includes per-request input/cache/output usage and payload hashes/character counts. Raw isolated smoke payloads were inspected under `/tmp/discovery-304`; they are reproducible with `bun scripts/code-mode-route-smoke.ts PROVIDER/MODEL OUTPUT_DIR explicit` (or `implicit`, `coding`). No provider/authentication replacement was used.

| Model       | Scenario               | Requests | Provider input total | Cached input | Output | Request input characters, first → last | Final context tokens |
| ----------- | ---------------------- | -------: | -------------------: | -----------: | -----: | -------------------------------------: | -------------------: |
| GPT-5.6 Sol | Explicit browsing      |        8 |               173942 |        86528 |    798 |                           173 → 174806 |                46702 |
| GPT-5.6 Sol | Inferred browsing need |        5 |                70040 |        36992 |    434 |                            149 → 81671 |                28090 |
| GPT-5.6 Sol | Coding only            |        3 |                 7841 |         4736 |    246 |                             181 → 4704 |                 2774 |
| GPT-6 Astra | Explicit browsing      |        4 |                36397 |        19200 |    182 |                            173 → 49613 |                16760 |
| GPT-6 Astra | Inferred browsing need |        3 |                20627 |         6528 |    180 |                            149 → 31666 |                11577 |
| GPT-6 Astra | Coding only            |        2 |                 5014 |            0 |     73 |                              181 → 594 |                 2566 |

Provider input total includes Pi's `input + cacheRead + cacheWrite`, summed over model requests; cached tokens are a subset, not additional input. Context tokens use Pi's final usage accounting; serialized input character growth includes all conversation/tool output and is not a token measure. Every browsing scenario returned the exact complete retained web contract in a separate observation before any web delegate. Each incurred one documentation lookup/model round trip. Both coding scenarios avoided web help and web calls. GPT-5.6's coding scenario is recorded as failed overall: it first tried `node -e`, the deliberately restrictive smoke UI denied it, and it recovered by computing 385 directly in `exec`. No discovery-order behavioral errors were observed in these six runs.

With all five capabilities available, grammar-mode exec help shrank from 14,372 to 7,444 characters: 6,928 characters, roughly 1,732 tokens under the local four-characters-per-token heuristic. These are description character measurements, not provider token savings. No paired eager baseline or repeated performance benchmark was run; the observed cache reads do not establish an improvement. Web output and extra requests can dominate total usage, as these runs show. Work/custom and live structured-fallback routes were unavailable; no substitute credentials/routes were tried. A first sandboxed request could not reach the configured service; the recorded live runs used normal network access.

Final `bun check` passed: 109 files / 1,342 tests passed (4 skipped), then 7 Bun-specific files / 89 tests passed. Focused real-host nested, registration, and transport checks also passed. No unit tests solely for prompt text were added.
