...
Retain bash-gate command classification and its automated-review or permission-UI paths for nested shell execution. Retire RTK throughout Pi-bites in a separate preparatory change before the Code Mode migration, so the new dispatcher does not inherit command-rewriting or output-filtering machinery that will subsequently be removed.
